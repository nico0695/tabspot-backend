...
import { ChangeStatusDto } from '../dto/requests/change-status.dto';
...
export class AdminUsersController {
  ...
  async list(@Query() query: ListAdminUsersDto): Promise<{
    ...
    const { items, totalCount } = await this.adminUsersService.listUsers(query);
    return {
      data: items,
      pageInfo: {
        page: query.page,
  ...
  async changeRole(
    ...
    return this.adminUsersService.changeUserRole(id, body.role, user.id);
  }
  ...
  async changeStatus(
    ...
    return this.adminUsersService.changeUserStatus(id, body.status, user.id);
  }
}
